      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="mb-5">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-slate-100 placeholder-slate-600
              focus:outline-none focus:ring-2 focus:ring-brand/50 focus:border-brand/60 transition-all"
          />
        </div>
        ...
        >
          ...
          {isDragActive ? (
            ...
            <>
              <p className="font-semibold text-slate-200 mb-1 text-lg">
                Arraste e solte as fotos
              </p>
              <p className="text-sm text-slate-500">
                ou <span className="text-brand underline cursor-pointer">clique para selecionar</span>
              </p>
        ...
        {files.length > 0 && (
          <div className="flex items-center justify-between mb-2 px-1">
            <p className="text-sm text-slate-400">
              ...
              {files.length >= 3 && <span className="ml-2 text-brand"><Check className="w-3.5 h-3.5 inline" /></span>}
            </p>
        ...
        {files.length > 0 && (
          <div className="bg-slate-800/60 border border-slate-700 rounded-2xl overflow-hidden mb-5 max-h-60 overflow-y-auto">
            {files.slice(0, 50).map((file, i) => (
              <div
                key={`${file.name}-${i}`}
                className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-700/50 last:border-0 hover:bg-slate-700/30 transition-colors"
              >
            ...
            {files.length > 50 && (
              <div className="px-4 py-2.5 text-xs text-slate-600 text-center">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-brand text-slate-900 font-bold py-4 rounded-xl text-lg
            ...
            focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
        >
          {submitting ? (
